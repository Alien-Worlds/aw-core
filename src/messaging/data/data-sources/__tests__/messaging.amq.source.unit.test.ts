...
import { ConnectionState } from '../../../domain/messaging.enums';
...
describe('Amq instance Unit tests', () => {
  ...
  it('"createChannel" should create channel correctly and assert queues "action", "aw_block_range", "recalc_asset"', async () => {
    ...
    await amq.init();
    const createChannelMock = jest.spyOn((amq as any).connection, 'createChannel');
    ...
    expect(assertQueueMock).toBeCalledWith('MessageQueue.AlienWorldsBlockRange', {
      durable: true,
    });
  ...
  it('"handleConnectionClose" should call Amqp.connect when connectionState is "closing"', async () => {
    const amq = new MessagingAmqSource('', { queues: [], prefetch: 1 }, console);
  ...
  it('"handleConnectionClose" should....', async () => {
    const amq = new MessagingAmqSource('', { queues: [], prefetch: 1 }, console);
    const reconnectMock = jest.spyOn(amq as any, 'reconnect').mockImplementation();
  ...
  it('"send" should trigger channel.sendToQueue', async () => {
    ...
    await amq.send('MessageQueue.Action', Buffer.from('')).catch(e => console.log(e));
  ...
  it('"addConnectionStateHandler" should assign handler to the connection state', async () => {
    ...
    amq.addConnectionStateHandler(ConnectionState.Online, async () => {});
    expect((amq as any).connectionStateHandlers.has(ConnectionState.Online)).toBeTruthy();
  });
  ...
  it('"removeConnectionStateHandlers" should remove handler assigned to the state', async () => {
    ...
    expect((amq as any).connectionStateHandlers.has(ConnectionState.Online)).toBeFalsy();
  });
